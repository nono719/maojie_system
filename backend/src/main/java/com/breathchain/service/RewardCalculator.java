 ...
 * 规则：
 *   1) 完成率 < 60%        → 不发奖
 *   2) 完成率 60-79%       → base × 0.5
 *   3) 完成率 80-89%       → base × 0.8
 *   4) 完成率 90-100%      → base × 1.0
 *   5) 评分 ≥ 90           → 在上一步基础上 × 1.2 "高质量训练加成"
 *   6) 连续打卡 ≥ 3 天      → × 1.3  "毅力之星"
 *   7) 连续打卡 ≥ 7 天      → × 1.5  "钢铁意志"（覆盖 6）
 *
...
public class RewardCalculator {
    ...
    public RewardDetail calc(BreathingTask task, TrainingRecord record, int streakDays) {
        ...
        if (completion.compareTo(BigDecimal.valueOf(60)) < 0) {
            return b.amount(BigDecimal.ZERO).reasons(List.of("完成率不足 60%，本次不发放奖励")).build();
        } else if (completion.compareTo(BigDecimal.valueOf(80)) < 0) {
            completionFactor = new BigDecimal("0.5"); completionTier = "良好 (60-79%)";
        } else if (completion.compareTo(BigDecimal.valueOf(90)) < 0) {
            completionFactor = new BigDecimal("0.8"); completionTier = "优秀 (80-89%)";
        } else {
